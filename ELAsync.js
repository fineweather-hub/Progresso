#!/usr/bin/env node
// ELAsync.js — Fetches all 3 Notion DBs and writes public/data.json
// Run locally:  NOTION_TOKEN=secret_xxx node ELAsync.js
...
const SUBJECT = "ELA"; // filter value — change to fetch a different subject
...
// Compound filter: subject = ELA AND status != Deferred AND status is not empty
...
// Skills only need subject + deferred filter (no status=empty guard needed, always set)
  ...
  const standards = stdRows.map(row => {
    ...
    return {
      ...
      lcs: rel(p["Learning Components"])
        .map(id => lcById[id])
        .filter(Boolean), // only LCs that passed the LC filter
    };
  });

  // Sort: domain order, then by code
